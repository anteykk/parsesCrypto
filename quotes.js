module.exports = {
  send(){
    ...
    let quites = require(`./models/quotes`);
    ...
    let scrape = async () => {
      ...
      const browser = await puppeteer.launch({
        ...
        */
        ...
        await page.goto('https://ru.tradingeconomics.com/currencies', {waitUntil: `networkidle2`, timeout: 0});
      ...
      // ПАРСИНГ КАТЕГОРИИ ВАЛЮТЫ
       ...
       // await page.waitForSelector("#pagemenutabs > li:nth-child(3) > a")
       /*
       Решения с проверкой наличия кнопки навигации
       let check1 = await page.evaluate(() => {
        return document.querySelector(`#pagemenutabs > li:nth-child(3) > a`).innerText;
       })
       console.log(check1);
       */
        ...
        let check1 = async()=>{
          try {
            await page.waitForSelector("#pagemenutabs > li:nth-child(3) > a",{timeout: 40000})  
          } catch (e) {
            await page.reload();
            console.log(e);
            console.log(`ВОЗНИКЛА ОШЫБКА, В TRY ПЕРЕЗАПУСКАЮ`)
            await check1();
          }
        }
       await check1()
        ...
        await page.click('#pagemenutabs > li:nth-child(3) > a', {waitUntil: `networkidle2`, timeout: 0});
      ...
      //await page.waitForSelector("#pagemenutabs > li:nth-child(4) > a")
      ...
      let check2 = async()=>{
        try {
          await page.waitForSelector("#pagemenutabs > li:nth-child(4) > a",{timeout: 40000})  
        } catch (e) {
          await page.reload();
          console.log(e);
          console.log(`ВОЗНИКЛА ОШЫБКА, В TRY ПЕРЕЗАПУСКАЮ`)
     ...
     await check2()
      ...
      await page.click('#pagemenutabs > li:nth-child(4) > a', {waitUntil: `networkidle2`, timeout: 0});
    ...
    //await page.waitForSelector("#pagemenutabs > li:nth-child(5) > a")
    let check3 = async()=>{
      try {
        await page.waitForSelector("#pagemenutabs > li:nth-child(5) > a",{timeout: 40000})  
      } catch (e) {
        await page.reload();
        console.log(e);
        console.log(`ВОЗНИКЛА ОШЫБКА, В TRY ПЕРЕЗАПУСКАЮ`)
   ...
   await check3()
    ...
    await page.click('#pagemenutabs > li:nth-child(5) > a', {waitUntil: `networkidle2`, timeout: 0});
    ...
    scrape().then((value) => {
      ...
      let hentai = async()=>{
        ...
        await quites.deleteOne({})
        .then(()=>{});

        await new quites({
          matket: [
            {
              currencies: allNeedPage.currencies,
              products:  allNeedPage.products,
              bonds: allNeedPage.bonds,
              crypto: allNeedPage.crypto
            }
          ],
          time: time()
        })
     ...
     // console.log(allNeedPage)        
        ...
        console.log(error);
        console.log(`ВОЗНИКЛА ОШЫБКА В THEN ПРИ СОРТИРОВКЕ ДАННЫХ`)
      }
